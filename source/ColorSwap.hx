...
class ColorSwap {
	public var forceColor(default, set):Bool = false;
	public var forcedColor(default, set):FlxColor = FlxColor.WHITE;
	public var shader(default, null):ColorSwapShader = new ColorSwapShader();
	public var hue(default, set):Float = 0;
	public var saturation(default, set):Float = 0;
	public var brightness(default, set):Float = 0;

	private function set_forceColor(value:Bool):Bool {
		forceColor = value;
		shader.forceColor.value = [value];
		return value;
	}

	private function set_forcedColor(value:FlxColor):FlxColor {
		forcedColor = value;
		shader.forcedColor.value = [value.redFloat, value.greenFloat, value.blueFloat];
		return value;
	}
	...
	public function new()
	{
		...
		shader.forceColor.value = [false];
		shader.forcedColor.value = [1.0, 1.0, 1.0];
	}
}
...
class ColorSwapShader extends FlxShader {
	@:glFragmentSource('
		...
		void main()
		{
			...
			if (forceColor) {
				color = vec4(forcedColor * color.a, swagColor[3]);
			} else {
