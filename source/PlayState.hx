...
class PlayState extends MusicBeatState
{
	...
	public var unspawnDatas:Array<ChartNoteData> = []; // ADD THIS
	...
	override public function create()
	{
		...
		if (ClientPrefs.data.disableGC)
			cpp.vm.Gc.setMinimumFreeSpace(1024 * 1024 * 1024 * 16); // 16GB before GC triggers;
		...
		if (isStoryMode)
		{
			detailsText = "bad Mode: " + WeekData.getCurrentWeek().weekName;
		}
		else
		{
			detailsText = "normal mode";
		}

		// String for when the game is paused
		detailsPausedText = "why are you paused - " + detailsText;
		#end
		...
		npsText = new FlxText(0, healthBarBG.y + 56, FlxG.width, "", 16);
		npsText.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		var noteMap:Map<String, ChartNoteData> = [];
		for (section in songData.notes)
		{
			for (i in 0...section.sectionNotes.length)
			{
				...
				if (ClientPrefs.data.clearGhostNotes) {
					final mapKey:String = leNoteData.strumLine + '_' + leNoteData.id + '_' + Math.round(leNoteData.time);
					if (noteMap.exists(mapKey))
					{
						var existing = noteMap.get(mapKey);
						existing.dispose();
						noteDatas.remove(existing);
						ghostNotesCleared++;
					}
					noteMap.set(mapKey, leNoteData);
				}
		...
		{
			unspawnDatas.push(note);
		}
		unspawnDatas.sort(function(a:ChartNoteData, b:ChartNoteData):Int {
	...
	{
		npsArray = npsArray.filter(function(t) return Conductor.songPosition - t < 1000);
		currentNPS = npsArray.length;
		if (currentNPS > peakNPS) peakNPS = currentNPS;
		...
		if (unspawnDatas.length > 0 || unspawnNotes[0] != null)
		{
			...
			while (spawnIndex < unspawnDatas.length && unspawnDatas[spawnIndex].time - Conductor.songPosition < time * 2)
			{
				...
				spawnIndex++;
				if (spawnIndex > 1000) {
					unspawnDatas.splice(0, spawnIndex);
					spawnIndex = 0;
				}
				...
				var swagNote:Note = new Note(noteData.time, noteData.id ?? 0, oldNote);
				swagNote.mustPress = noteData.strumLine == 1;
				swagNote.sustainLength = noteData.sLen ?? 0;
				swagNote.gfNote = noteData.isGfNote ?? false;
				swagNote.noteType = noteData.type ?? '';
				...
				if (floorSus > 0)
				{
					...
					{
						oldNote = unspawnNotes[unspawnNotes.length - 1];
						var sustainNote:Note = new Note(noteData.time + (Conductor.stepCrochet * susNote) +
							(Conductor.stepCrochet / FlxMath.roundDecimal(songSpeed, 2)),
							noteData.id, oldNote, true);

						sustainNote.mustPress = noteData.strumLine == 1;
			...
			if (unspawnNotes[0] != null)
			{
				...
				while (unspawnNotes.length > 0 && unspawnNotes[0].strumTime - Conductor.songPosition < time)
				{
					...
					if (ClientPrefs.data.noteQuantization && dunceNote.colorSwap != null) {
						dunceNote.colorSwap.hue = getQuantizationHue(dunceNote.strumTime);
					}
	...
	function getQuantizationHue(strumTime:Float):Float {
		var stepCrochet:Float = Conductor.stepCrochet;
		var beat:Float = strumTime / (stepCrochet * 4);
		var fraction:Float = beat - Math.floor(beat);

		var divisions:Array<Float> = [1/1, 1/2, 1/4, 1/8, 1/16, 1/32, 1/48, 1/64];
		var hues:Array<Float> = [0.0, 0.08, 0.17, 0.25, 0.5, 0.67, 0.75, 0.83];

		var closest:Int = 0;
		var closestDiff:Float = 999;
		for (i in 0...divisions.length) {
			var snapped:Float = Math.round(fraction / divisions[i]) * divisions[i];
			var diff:Float = Math.abs(fraction - snapped);
			if (diff < closestDiff) {
				closestDiff = diff;
				closest = i;
			}
	...
	function openChartEditor()
	{
		...
		#if hxdiscord_rpc
		DiscordClient.changePresence("carting", null, null, true);
		#end
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false)
	{
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if (ret != FunkinLua.Function_Stop)
			{
				...
				if (ClientPrefs.data.disableGC)
					cpp.vm.Gc.setMinimumFreeSpace(0);
				...
				DiscordClient.changePresence("he missed too many notes again... - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
	...
	function goodNoteHit(note:Note):Void
	{
		...
		{
			trace('disableNoteGlow: ' + ClientPrefs.data.disableNoteGlow);
			var spr = playerStrums.members[note.noteData];
			if (spr != null)
				spr.playAnim(ClientPrefs.data.disableNoteGlow ? 'static' : 'confirm', true);
		}
